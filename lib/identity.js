...
/**
 * The fingerprints of the keys WorkBuddy has stored.
 *
 * A sibling of {@link parseOpencodeAuthKeys} rather than a reuse, because the
 * store is a different shape: opencode maps provider → key, while WorkBuddy's
 * `models.json` is a *list* of its own configured providers. The verdict shape
 * is deliberately identical so the two feed the same matcher contract.
 *
 * `id` is not decoration. WorkBuddy addresses one of its own configured
 * providers as `custom-local:<id>` in every usage record it writes, so `id` is
 * the join key between a credential and the rows it paid for (DESIGN §4.2.2).
 * An entry carrying no `id` therefore cannot be joined to anything, and that is
 * *reported* rather than skipped: quietly dropping it would shrink the month
 * with nothing on screen to say why.
 *
 * Entries without an `apiKey` are skipped silently, mirroring opencode's
 * treatment of an OAuth provider: having no key at all is a different fact from
 * having a key that cannot be used.
 *
 * @param parsed - the decoded `models.json`.
 * @returns one verdict per entry that stores a key.
 */
export function parseWorkbuddyModelKeys(parsed) {
    const found = [];
    ...
    for (const entry of parsed) {
        ...
        if (id.length === 0) {
            found.push({ provider: named.length > 0 ? named : '(unnamed)', ok: false, reason: 'noModelId' });
            continue;
        }
        const fingerprint = fingerprintOfKey(entry.apiKey);
        if (!fingerprint.ok) {
            found.push({ provider: id, ok: false, reason: fingerprint.reason });
            continue;
        }
        found.push({
            provider: id,
...
/**
 * Which tracked key WorkBuddy's configured providers resolve to, and every
 * model id that addresses it.
 *
 * Every id has to come back, not just the first match. Measured on a real
 * store: two entries (`deepseek-v4-flash`, `deepseek-v4-pro`) hold the very same
 * key, so a matcher that stopped at the first one would attribute only the rows
 * that happen to name it and silently drop the rest.
 *
 * @param options - match inputs.
 * @param options.workbuddyKeys - verdicts from {@link parseWorkbuddyModelKeys}.
 * @param options.keys - resolved tracked keys.
 * @returns the matched key and its model ids, or `undefined` when WorkBuddy is on a different key.
 */
export function matchWorkbuddyKeys(options) {
    const { workbuddyKeys, keys } = options;
    const modelIds = [];
    let matched;
    for (const entry of workbuddyKeys) {
        if (!entry.ok) {
            continue;
        }
        const key = keys.find(candidate => candidate.fingerprint === entry.fingerprint);
        ...
        if (!modelIds.includes(entry.modelId)) {
            modelIds.push(entry.modelId);
        }
    }
    return matched === undefined ? undefined : { key: matched, modelIds };
}
