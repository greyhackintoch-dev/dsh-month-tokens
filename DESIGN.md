...
- **数据源就是 `days`，不新增载荷字段。** `days` 的契约是按月过滤的（§4.3），所以**月初只能画出已有的那几天**：3 号三个点，7 号起才是七个点。图**按实际画出的天数标注**（`最近 3 天` + `10/1–10/3`），**绝不把缺的那几天补成 0** —— 补零等于宣称「那天没花钱」，而事实是「那天没有记录」，这是两句不同的话。
- **不做跨月滚动窗口。** 那需要四个读取器各自保留 7 天、并让 §4.3 的跨机快照携带第二个窗口——为一张图去动那条刚刚修好跨月缺陷的契约，代价与收益不成比例。若将来真要跨月连续，正确的做法是在快照里加一个显式的 `recent` 字段，而不是放松 `days` 的月份过滤。
- **不引入图表库。** 客户端只依赖平台种子模块（`react` / `react-dom`），一条折线就是一个 `path`。viewBox 宽度取的就是面板内容宽度（288），因此保持等比缩放——用 `preserveAspectRatio="none"` 会把圆点拉成椭圆、把发丝线拉粗。
- **少于两个点不画。** 一个点是点，不是趋势；为它造一根轴，说出来的比数据本身还多。
- 每个点带一条不可见的悬停列，其 `<title>` 给出该日精确值——288px 宽里塞七个数字，不如让鼠标去问。
