...
- 面板**只回答两个问题：总量多少、来自哪里**（`client/client.js` 的取舍，2026-09 由使用者定）：
  - 保留：headline 总量、`我的 key` 一行（含该 key 全时段累计）、**最近 7 天折线图**、**每台机器的本月贡献**（陈旧者保留数字并标注）、以及 `本月消耗` 里的**各平台**（本机 DSH / opencode / Pen / WorkBuddy；后三者只在真的有记录时才出现）。
...
### 4.4.1 最近 7 天折线图（2026-09 追加）

画在 `我的 key` 自己的两组数字（本月、全时段）之下、机器行之上：**图是关于这把 key 的**，下面的实例行回答的是另一个轴（谁贡献的，不是怎么走的）。

**坐标轴是日历日，不是「有记录的那几天」。** 这条在开发中被实测纠正过一次：第一版只取 `days` 里存在的键，于是 9/16–9/24 被画成横跨九天的七个点——**空隙被静默吃掉了，轴的间距变成「用过的天」而不是「天」**。正确读法是分段的：

- **月内缺 key = 真零**。`days` 按月过滤、折叠走过当月每一天，所以当月某天没有记录就是那天没花钱。**画成零，不许跳过。**
- **月初之前 = 未知**。折叠根本没看过那里，所以那几天**留空而不是补零**，窗口因此变短，图按实际画出的天数标注（`最近 3 天` + `10/1–10/3`）。

锚点取 `days` 里**最后一天有记录的日期**，不是「今天」：清晨还没有消耗时，锚在今天就等于把窗口推到一个尚未发生的日子上。

**刻度只有「日」，没有月份——而且是不可达代码驱动的取舍，不是偷懒。** `days` 按月过滤（§4.3），窗口又被钳在那个月内，所以这条轴上的每一天**必然同月**，`M/D` 形式永远触发不到。范围标签已经把月份说了。真正会破坏这个不变量的，是将来那个跨月 `recent` 窗口（下面第三条），那才是需要改这一行的时候。

其余四条：

- **不做跨月滚动窗口。** 那需要四个读取器各自保留 7 天、并让 §4.3 的跨机快照携带第二个窗口——为一张图去动那条刚刚修好跨月缺陷的契约，代价与收益不成比例。若将来真要跨月连续，正确做法是在快照里加一个显式的 `recent` 字段，而不是放松 `days` 的月份过滤。
- **不引入图表库。** 客户端只依赖平台种子模块（`react` / `react-dom`），一条折线就是一个 `path`。绘图区 58px、刻度带 15px，viewBox 宽取的就是面板内容宽度（288），因此 1:1 渲染不缩放——用 `preserveAspectRatio="none"` 会把圆点拉成椭圆、把发丝线拉粗。图是唯一的**通栏**元素（渲染在缩进的行包裹器之外），正是为了让 viewBox 宽度与内容宽度继续相等。
- **少于两个点不画。** 在日历轴上，这只在**每月 1 号**发生——窗口里没有更早的一天可画。一个孤点配一根轴，说出来的比数据本身还多。
- 每个点带一条不可见悬停列，其 `<title>` 给出该日精确值——把七个数字塞在 288px 宽的图下面，不如让鼠标去问。
...
7. **折线图**（§4.4.1）：七列、最旧在前、超过七天时丢掉最旧那天；`aria-label` 里的范围与实际画出的天数一致；少于两个点不画；月初只有三天时标签写「最近 3 天」而不是补零到七天。
